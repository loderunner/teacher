...
[Persist Syllabus Draft](./persist-syllabus-draft.md) plan must be in place before this plan
is implemented. The delta transport relies on the database being the source of truth.
...
**Prerequisite**: the `messages` table must be populated for chapter sessions. Chapter-chat
message persistence is outside the scope of the Persist Syllabus Draft plan and must be
implemented before the delta transport can be enabled on this route.
...
- The **synthetic user message** injected by `chapter-page.tsx` via `setMessages` (the
  "I applied the suggested syllabus change." message) must be explicitly saved to the DB when
  the `proposeSyllabusChange` tool is applied. Currently this message only lives in client
  state. When the delta transport is active for chapter chat, this injection must also call a
  server action or endpoint to persist the synthetic message, or the next AI turn will be
  missing that context. This is a non-trivial edge case that must be resolved as part of chapter
  chat delta enablement.
...
Add `prepareSendMessagesRequest` to `DefaultChatTransport`. This is backward-compatible at the
transport level but changes the request body schema, so the API route must be updated in the
same deployment.
...
Repeat Steps 2–3 for the chapter chat route. Address the synthetic-message edge case (§8)
